...
module.exports = function({ blobServiceClient, KYC_CONTAINER_NAME, azureEnabled = false }) {
    ...
    router.post('/verify-identity', userAuth, async (req, res) => {
        ...
        try {
            // 1. Sanitize input: Trim spaces and remove all dashes from the ID number
            const sanitizedFullName = fullName.trim();
            const sanitizedIdentityNumber = identityNumber.trim().replace(/-/g, ''); // Removes dashes like in CNIC

            // 2. Check for existing user with the SAME sanitized Identity Number ONLY
            ...
            if (existingUser) {
                ...
                return res.status(409).json({
                    success: false,
                    message: 'This identity number is already associated with another account.',
                    reason: 'already_exists'
                });
            }

            // 3. Identity is new, save it to the current user's profile
            ...
            user.identityNumber = sanitizedIdentityNumber; // Save sanitized ID without dashes
            ...
            // Handle unique index errors specifically
            if (error.code === 11000) { 
                 return res.status(409).json({ 
                    success: false, 
    ...
    // ----------------------------------------------------------------------
    // [POST] /api/kyc/upload - Handle file upload and status update (includes selfie)
    // ----------------------------------------------------------------------
